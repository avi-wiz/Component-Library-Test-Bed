...
const renderMetricCard = (data) => {
  ...
  const tc = (trend && trend.direction === 'up') ? 'var(--success)' : (trend && trend.direction === 'down') ? 'var(--danger)' : 'var(--text2)';
  ...
  return (
    <div className="metric-card" style={{ maxWidth: 300 }}>
      ...
      {trend && <div className="metric-trend" style={{ color: tc }}>{arr} {Math.abs(trend.percent)}% <span style={{ color: 'var(--text3)', fontWeight: 400, fontSize: 10.5 }}>{trend.period}</span></div>}
        ...
        {cards.map((c, i) => {
          ...
          return (
            <div key={i} className="metric-card" style={{ flex: '1 1 160px', position: 'relative' }}>
              ...
              {c.trend && <div className="metric-trend" style={{ color: tc, fontSize: 11 }}>{arr} {Math.abs(c.trend.percent)}% <span style={{ color: 'var(--text3)', fontWeight: 400, fontSize: 10 }}>{c.trend.period}</span></div>}
            </div>
          );
        })}
      </div>
      ...
      {sections.map((sec, si) => (
        <div key={si} style={{ borderBottom: '1px solid var(--border)' }}>
          <div onClick={() => setCollapsed(c => ({ ...c, [si]: !c[si] }))} style={{ padding: '11px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }}>
            <span style={{ fontSize: 11.5, fontWeight: 700, color: 'var(--text2)', textTransform: 'uppercase', letterSpacing: '.6px' }}>{sec.title}</span>
            <span style={{ color: 'var(--text3)', fontSize: 12 }}>{collapsed[si] ? '›' : '⌄'}</span>
          </div>
          {!collapsed[si] && <div style={{ padding: '0 16px 12px' }}>
            {(sec.fields || []).map((f, fi) => (
              <div key={fi} style={{ display: 'flex', gap: 12, padding: '5px 0', borderBottom: fi < sec.fields.length - 1 ? '1px solid var(--border2)' : 'none' }}>
                <span style={{ fontSize: 11.5, color: 'var(--text3)', minWidth: 130, flexShrink: 0 }}>{f.key}</span>
                <span style={{ fontSize: 11.5, color: 'var(--text)' }}>{f.value}</span>
              </div>
            ))}
          </div>}
        </div>
      ))}
      {actions.length > 0 && <div style={{ padding: '12px 16px', display: 'flex', gap: 8, flexWrap: 'wrap', background: 'var(--surface2)' }}>{actions.map((a, i) => renderAction(a, i))}</div>}
    </div>
  );
};
    ...
    <div>
      <div style={{ overflowX: 'auto', borderRadius: 10, border: '1px solid var(--border)' }}>
        <table className="tbl">
          ...
          <tbody>
            {sorted.length === 0 ? <tr><td colSpan={columns.length} style={{ textAlign: 'center', color: 'var(--text3)', padding: 24 }}>{emptyState}</td></tr>
        ...
        return (
          <div key={stage.id} className="kb-col">
            <div className="kb-col-hdr">
              <div className="kb-col-name" style={{ color: stage.color || 'var(--text)' }}>{stage.name}</div>
            ...
            <div className="kb-cards">
              ...
              {sc.length === 0 && <div style={{ fontSize: 11, color: 'var(--text3)', textAlign: 'center', padding: 12 }}>No cards</div>}
            </div>
          </div>
        );
      })}
...
const renderCustomer360 = (data) => {
  ...
  const riskColors = { healthy: 'var(--success)', watch: '#f59e0b', at_risk: 'var(--danger)', churned: 'var(--text2)' };
  const rc = (riskIndicator && riskColors[riskIndicator.level]) || 'var(--text2)';
  return (
    <div>
      {riskIndicator && <div style={{ background: rc + '15', border: `1px solid ${rc}40`, borderRadius: 8, padding: '9px 14px', marginBottom: 14, display: 'flex', gap: 10, alignItems: 'flex-start' }}>
        ...
        <div><div style={{ fontSize: 12, fontWeight: 700, color: rc, textTransform: 'capitalize' }}>{(riskIndicator.level ? riskIndicator.level.replace('_', ' ') : '')} Customer</div>
          <div style={{ fontSize: 11.5, color: 'var(--text2)', marginTop: 2 }}>{riskIndicator.reason}</div></div>
      </div>}
        ...
        <div>
          ...
          {[['Sales Rep', (profile && profile.salesRep)], ['Price List', (profile && profile.priceList)], ['Territory', (profile && profile.territory)]].map(([k, v]) => (
            <div key={k} style={{ display: 'flex', gap: 10, fontSize: 12, marginBottom: 5 }}>
              <span style={{ color: 'var(--text3)', minWidth: 80 }}>{k}</span><span style={{ color: 'var(--text)' }}>{v}</span>
            </div>
          ))}
        </div>
        <div>
          <div style={{ fontSize: 10.5, color: 'var(--text3)', fontFamily: 'JetBrains Mono,monospace', marginBottom: 6, textTransform: 'uppercase', letterSpacing: .8 }}>Revenue Trend</div>
      ...
      {(recentOrders || []).map((o, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '7px 0', borderBottom: '1px solid var(--border)' }}>
          ...
          <span style={{ fontSize: 11, color: 'var(--text3)', flex: 1 }}>{o.date}</span>
          <StatusBadge {...o.status} size="sm" />
          <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)', fontFamily: 'JetBrains Mono,monospace' }}>${(o.value ? o.value.toLocaleString() : '0')}</span>
        </div>
      ))}
      ...
      {(openTasks || []).map((t, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0', borderBottom: '1px solid var(--border)', fontSize: 12 }}>
          ...
          <span style={{ color: 'var(--text3)', marginLeft: 'auto', fontFamily: 'JetBrains Mono,monospace', fontSize: 10.5 }}>Due {t.due}</span>
        </div>
      ))}
    </div>
  );
...
const renderProductCard = (data) => {
  ...
  const sc = { in_stock: 'var(--success)', low_stock: '#f59e0b', out_of_stock: 'var(--danger)' }[(availability && availability.status)] || 'var(--text2)';
  ...
  return (
    <div className="product-card" style={{ maxWidth: 240 }}>
      <div className="product-img" style={{ background: 'var(--border2)' }}>🛍️</div>
...
const renderProductGrid = (data) => {
  ...
  const sc = { in_stock: 'var(--success)', low_stock: '#f59e0b', out_of_stock: 'var(--danger)' };
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(150px,1fr))', gap: 10 }}>
        {products.map((p, i) => (
          <div key={i} className="product-card">
            <div className="product-img" style={{ background: 'var(--border2)', fontSize: 24 }}>📦</div>
            <div className="product-body">
      ...
      <div style={{ marginTop: 12, display: 'flex', justifyContent: 'space-between', fontSize: 11.5, color: 'var(--text3)', fontFamily: 'JetBrains Mono,monospace' }}>
...
const renderCompactList = (data) => {
  ...
  if (!items.length) return <div style={{ textAlign: 'center', color: 'var(--text3)', fontSize: 12, padding: 16 }}>{emptyState}</div>;
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 16, marginBottom: 14, flexWrap: 'wrap' }}>
        <div><div style={{ fontSize: 10.5, color: 'var(--text3)', marginBottom: 2 }}>Customer</div><div style={{ fontSize: 13, fontWeight: 700 }}>{(customer && customer.name)}</div></div>
        <div><div style={{ fontSize: 10.5, color: 'var(--text3)', marginBottom: 2 }}>Price List</div><div style={{ fontSize: 12, color: 'var(--text)' }}>{priceList}</div></div>
      </div>
      ...
      {container && (
        <div style={{ marginBottom: 14, background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, padding: '11px 14px' }}>
          <div style={{ fontSize: 10.5, color: 'var(--text3)', marginBottom: 8, fontFamily: 'JetBrains Mono,monospace', textTransform: 'uppercase', letterSpacing: .8 }}>Container Fill</div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11.5, marginBottom: 6 }}>
            <span style={{ color: 'var(--text2)' }}>{container.totalCft} / {container.cftCapacity} CFT</span>
            <span style={{ color: container.fillPercent > 90 ? 'var(--danger)' : container.fillPercent > 70 ? '#f59e0b' : 'var(--success)', fontWeight: 700 }}>{container.fillPercent}%</span>
          </div>
          <div className="prog-bar-wrap"><div className="prog-bar-fill" style={{ width: `${container.fillPercent}%`, background: container.fillPercent > 90 ? 'var(--danger)' : container.fillPercent > 70 ? '#f59e0b' : 'var(--success)' }} /></div>
        </div>
      )}
      {summary && (
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <div style={{ minWidth: 220 }}>
            {[['Subtotal', summary.orderTotal], ['Discount', summary.discount ? -summary.discount : null], ['Shipping', summary.shipping]].filter(([, v]) => v != null).map(([k, v]) => (
              <div key={k} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, padding: '4px 0', borderBottom: '1px solid var(--border)', color: 'var(--text2)' }}>
  ...
  return (
    <div>
      <div className="chip-bar">
        ...
        {chips.length === 0 && <span style={{ fontSize: 12, color: 'var(--text3)' }}>No active filters</span>}
      </div>
      <div style={{ marginTop: 10, fontSize: 11, color: 'var(--text3)', fontFamily: 'JetBrains Mono,monospace' }}>
